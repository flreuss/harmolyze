...
export class SimultaneousNotesArray {
  ...
  constructor(voicesArray) {
    ...
    while (existsUnclassifiedNote(voicesArray, current)) {
      ...
      const minCountTotal = Math.min(...current.map((voice) => voice.countTotal));
      //Workaround, if one voice finishes earlier than another (i. e. in broken abc strings)
      countTotal = minCountTotal <= countTotal ? countTotal + (1/64) : minCountTotal;
    }
...
//simultaneousNotesArray kann übergeben werden, um Rechnerei zu ersparen
export function chordOf(elem, voicesArray, simultaneousNotesArray) {
