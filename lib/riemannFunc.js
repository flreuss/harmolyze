...
export default class RiemannFunc {
  ...
  constructor(
    ...
    const BASE_FUNCS = ["m", "min", "minor"].includes(mode)
      ? BASE_FUNCS_MINOR
      : BASE_FUNCS_MAJOR;

    this.baseFunc = BASE_FUNCS[baseFunc];
  ...
  static validBaseNotes = [...Array(9).keys()].slice(1);
   ...
   */
  static fromString(riemannString, mode) {
...
export class CondensedFunc extends RiemannFunc {
  ...
  constructor(
    ...
    super(baseFunc, addTones, base, isSecondaryDominant, incomplete, mode);
    this.given = given;
  }
  ...
  static fromString(riemannString, mode) {
    const riemannFunc = RiemannFunc.fromString(riemannString, mode);
