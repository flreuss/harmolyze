...
export function calculatePoints(abc) {
    ...
    if (elem.abcelem.chord) {
      points += Math.min(
        elem.abcelem.chord.map((chord) => {
          ...
          const result = condensedFunc.given
            ? 0
            : alreadySeen.includes(condensedFunc.toString())
            ? condensedFunc.getPoints() / 2
            : condensedFunc.getPoints();
          alreadySeen.push(condensedFunc.toString());
          return result;
        })
      );
    }
  ...
  points *= 1 + 0.1 * keySignature.accidentals.length;
